...
AddEventHandler("uh-buffs:stamina", function(pEnabled, pPercent)
...
AddEventHandler("uh-buffs:setJobLuckMultiplier", function(pJobLuckMultiplier)
  jobLuckMultiplier = 1 + pJobLuckMultiplier + 0.0
  TriggerServerEvent("uh-buffs:setBuffValue", "jobluck", jobLuckMultiplier)
end)
...
AddEventHandler("uh-buffs:setAlertLevelMultiplier", function(pAlertLevelMultiplier)
...
AddEventHandler("uh-buffs:inDoubleBuffZone", function(pBool)
  ...
  Citizen.CreateThread(function()
    ...
    while isSitting and inRestaurantBuffZone do
      if minuteCount >= 5 then
        ...
        TriggerEvent("uh-buffs:applyBuff", "restaurant_double", {{ buff = "double", percent = percent }})
      end
      Citizen.Wait(60000)
