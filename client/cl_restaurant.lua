...
local curJob = "none"
RegisterNetEvent("uh-jobmanager:playerBecameJob")
AddEventHandler("uh-jobmanager:playerBecameJob", function(job)
  curJob = job
end)
  ...
  ["resfooditem"] = {
    ...
    restaurantBaseBuff = {
      ["burger_shot"] = function(info)
        ...
        TriggerEvent("uh-buffs:applyBuff", "burger_shot", { { buff = "stressblock", percent = 1.0, timeOverride = 180 * 60000 } })
        TriggerEvent("hadsugar")
        return processInfo(info)
      end,
      ["rooster"] = function(info)
        TriggerEvent("uh-buffs:applyBuff", "rooster", { { buff = "jobluck", percent = 1.0 } })
        return processInfo(info)
      end,
      ["uwu_cafe"] = function(info)
        TriggerEvent("uh-buffs:applyBuff", "uwu_cafe", { { buff = "strength", percent = 0.5, timeOverride = 60000 * 60 } })
        return processInfo(info)
      end,
      ["maldinis"] = function(info)
        TriggerEvent("uh-buffs:applyBuff", "maldinis", { { buff = "fullness", percent = 1.0 } })
        return processInfo(info)
      end,
    },
  },
...
AddEventHandler("uh-buffs:itemUsedSuccess", function(pSuccess)
  itemUseSuccess = pSuccess
end)
AddEventHandler("uh-inventory:itemUsed", function(pItem, pInfo)
  local config = buffConfig[pItem]
  if not config then return end
  itemUseSuccess = nil
  TriggerEvent("uh-inventory:attachPropPlayAnim", config.attachItem)
  ...
  TriggerEvent("uh-buffs:applyBuff", pItem, buffs)
